...
function getClientDataFromDb(id, callback){
  ...
  if (id == null) {
    ...
    sql = "SELECT c.id AS client_id, c.firstName, c.lastName, c.active, ci.streetAddress, ci.city, ci.state, ci.zipcode, ci.phone, ci.email, ci.registrationDate, json_build_object( 'training_stuff', json_agg(json_build_object('session_id', ts.id, 'session_name', ts.sessionname, 'session_description', ts.sessiondescription, 'session_sets', ts.sessionSets, 'session_reps', ts.sessionreps, 'session_active_flag',ts.active))) training_sessions FROM client AS c JOIN client_Info AS ci ON ci.clientid = c.id LEFT JOIN client_training_session AS cts ON cts.clientid = c.id LEFT JOIN training_session AS ts ON ts.id = cts.sessionid GROUP BY c.id, ci.streetaddress, ci.city, ci.state, ci.zipcode, ci.phone, ci.email, ci.registrationDate ORDER BY c.id ASC;"
